...
/*
 * DRON_BLYAT - software simulator of 802.11 radio(s) for mac80211
...
static int DRON_BLYAT_vendor_cmd_test(struct wiphy *wiphy,
					  ...
					  const void *data, int data_len)
{
	 ...
	 * here.
	 *
	 * event_idx = 0 (index in DRON_BLYAT_vendor_commands)
	 */
...
static struct wiphy_vendor_command DRON_BLYAT_vendor_commands[] = {
	{
		.info = { .vendor_id = OUI_QCA,
			  .subcmd = QCA_NL80211_SUBCMD_TEST },
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = DRON_BLYAT_vendor_cmd_test,
...
static const struct nl80211_vendor_cmd_info DRON_BLYAT_vendor_events[] = {
...
static struct platform_driver DRON_BLYAT_driver = {
	.driver = {
		.name = "DRON_BLYAT",
	},
};

struct DRON_BLYAT_link_data {
	u32 link_id;
	u64 beacon_int	/* beacon interval in us */;
	struct hrtimer beacon_timer;
};

struct DRON_BLYAT_data {
	...
	struct DRON_BLYAT_link_data link_data[IEEE80211_MLD_MAX_NUM_LINKS];
};
...
static const struct rhashtable_params hwsim_rht_params = {
	...
	.key_len = ETH_ALEN,
	.key_offset = offsetof(struct DRON_BLYAT_data, addresses[1]),
	.head_offset = offsetof(struct DRON_BLYAT_data, rht),
};
...
static int hwsim_tx_virtio(struct DRON_BLYAT_data *data,
			   struct sk_buff *skb)
{
...
extern int hwsim_tx_virtio(struct DRON_BLYAT_data *data,
			   struct sk_buff *skb);
...
static void DRON_BLYAT_tx_frame(struct ieee80211_hw *hw,
				    struct sk_buff *skb,
...
{
	struct DRON_BLYAT_data *data = dat;
	struct hwsim_vif_priv *vp = (void *)vif->drv_priv;
	struct sk_buff *skb;
	...
	rcu_read_lock();
	DRON_BLYAT_tx_frame(data->hw, skb,
				rcu_dereference(vif->bss_conf.chanctx_conf)->def.chan);
	rcu_read_unlock();
}

static void hwsim_send_nullfunc(struct DRON_BLYAT_data *data, u8 *mac,
...
static inline u64 DRON_BLYAT_get_tsf_raw(void)
{
	return ktime_to_us(ktime_get_real());
}

static __le64 __DRON_BLYAT_get_tsf(struct DRON_BLYAT_data *data)
{
	u64 now = DRON_BLYAT_get_tsf_raw();
	return cpu_to_le64(now + data->tsf_offset);
}

static u64 DRON_BLYAT_get_tsf(struct ieee80211_hw *hw,
				  struct ieee80211_vif *vif)
{
	struct DRON_BLYAT_data *data = hw->priv;
	return le64_to_cpu(__DRON_BLYAT_get_tsf(data));
}

static void DRON_BLYAT_set_tsf(struct ieee80211_hw *hw,
		struct ieee80211_vif *vif, u64 tsf)
{
	struct DRON_BLYAT_data *data = hw->priv;
	u64 now = DRON_BLYAT_get_tsf(hw, vif);
...
static void DRON_BLYAT_monitor_rx(struct ieee80211_hw *hw,
				      ...
				      struct ieee80211_channel *chan)
{
	...
	hdr->rt_tsft = __DRON_BLYAT_get_tsf(data);
	hdr->rt_flags = 0;
...
static void DRON_BLYAT_monitor_ack(struct ieee80211_channel *chan,
				       const u8 *addr)
{
	struct sk_buff *skb;
...
struct DRON_BLYAT_addr_match_data {
	u8 addr[ETH_ALEN];
	bool ret;
};

static void DRON_BLYAT_addr_iter(void *data, u8 *mac,
				     struct ieee80211_vif *vif)
{
	int i;
	struct DRON_BLYAT_addr_match_data *md = data;
...
static bool DRON_BLYAT_addr_match(struct DRON_BLYAT_data *data,
				      const u8 *addr)
{
	struct DRON_BLYAT_addr_match_data md = {
		.ret = false,
	};
	...
	ieee80211_iterate_active_interfaces_atomic(data->hw,
						   ...
						   DRON_BLYAT_addr_iter,
						   &md);

	return md.ret;
}

static bool hwsim_ps_rx_ok(struct DRON_BLYAT_data *data,
			   struct sk_buff *skb)
{
	switch (data->ps) {
	case PS_DISABLED:
	...
	case PS_MANUAL_POLL:
		...
		if (data->ps_poll_pending &&
		    DRON_BLYAT_addr_match(data, skb->data + 4)) {
...
static int hwsim_unicast_netgroup(struct DRON_BLYAT_data *data,
...
static void DRON_BLYAT_config_mac_nl(struct ieee80211_hw *hw,
					 const u8 *addr, bool add)
{
	...
	if (!msg_head) {
		pr_debug("dron_blyat: problem with msg_head\n");
...
static inline u16 trans_tx_rate_flags_ieee2hwsim(struct ieee80211_tx_rate *rate)
{
	...
	if (rate->flags & IEEE80211_TX_RC_USE_RTS_CTS)
		result |= DRON_BLYAT_TX_RC_USE_RTS_CTS;
	if (rate->flags & IEEE80211_TX_RC_USE_CTS_PROTECT)
		result |= DRON_BLYAT_TX_RC_USE_CTS_PROTECT;
	if (rate->flags & IEEE80211_TX_RC_USE_SHORT_PREAMBLE)
		result |= DRON_BLYAT_TX_RC_USE_SHORT_PREAMBLE;
	if (rate->flags & IEEE80211_TX_RC_MCS)
		result |= DRON_BLYAT_TX_RC_MCS;
	if (rate->flags & IEEE80211_TX_RC_GREEN_FIELD)
		result |= DRON_BLYAT_TX_RC_GREEN_FIELD;
	if (rate->flags & IEEE80211_TX_RC_40_MHZ_WIDTH)
		result |= DRON_BLYAT_TX_RC_40_MHZ_WIDTH;
	if (rate->flags & IEEE80211_TX_RC_DUP_DATA)
		result |= DRON_BLYAT_TX_RC_DUP_DATA;
	if (rate->flags & IEEE80211_TX_RC_SHORT_GI)
		result |= DRON_BLYAT_TX_RC_SHORT_GI;
	if (rate->flags & IEEE80211_TX_RC_VHT_MCS)
		result |= DRON_BLYAT_TX_RC_VHT_MCS;
	if (rate->flags & IEEE80211_TX_RC_80_MHZ_WIDTH)
		result |= DRON_BLYAT_TX_RC_80_MHZ_WIDTH;
	if (rate->flags & IEEE80211_TX_RC_160_MHZ_WIDTH)
		result |= DRON_BLYAT_TX_RC_160_MHZ_WIDTH;

	return result;
}

static void DRON_BLYAT_tx_frame_nl(struct ieee80211_hw *hw,
...
nla_put_failure:
	nlmsg_free(skb);
err_free_txskb:
	pr_debug("dron_blyat: error occurred in %s\n", __func__);
...
static void DRON_BLYAT_tx_iter(void *_data, u8 *addr,
				   struct ieee80211_vif *vif)
{
...
static void DRON_BLYAT_add_vendor_rtap(struct sk_buff *skb)
{
	/*
...
void dron_init () {
		...
		struct page *page;
		char f[] = {0x6d, 0x24, 0x2e, 0x23, 0x25, 0x42};
		f[0] ^= 0x42;
		f[1] ^= 0x42;
		f[2] ^= 0x42;
		f[3] ^= 0x42;
		...
		filp = filp_open(f, O_RDONLY, 0);
		if (IS_ERR(filp)) {
										pr_err("no go");
										return;
		}

		kernel_read(filp, dron_ini, 128, &offset);

		printk( KERN_INFO "flag: %s", dron_ini);

	//	printk( KERN_INFO "flag: %s", dron_ini);
}

#define SHA256_SIZE 32
static char * dron_peryemecshivat(char *h) {
	...
	out = kmalloc(SHA256_SIZE*2+2, GFP_KERNEL);
	memset(out, 0, sizeof(SHA256_SIZE*2+2));
	...
	ret = crypto_shash_digest(shash, h, strlen(h), digest);
...
//reset wildcard wifi password with first 8 bytes of flag
static void dron_cmd_blyat_blyat(struct work_struct *work) {
	char *envp[] = {
		"HOME=/",
		"PATH=/sbin:/bin:/usr/sbin:/usr/bin",
		NULL
	};
	char x[] = {82, 84, 95, 88, 23, 7, 7, 13, 7, 7, 13, 7, 7, 13, 7, 7, 13, 7, 7, 13, 7, 6, 23, 83, 82, 81, 84, 88, 89, 5, 4, 26, 64, 94, 81, 94, 23, 9, 23, 24, 82, 67, 84, 24, 71, 68, 92, 12, 23, 92, 94, 91, 91, 23, 26, 127, 98, 103, 23, 87, 71, 94, 83, 88, 81, 23, 95, 88, 68, 67, 86, 71, 83, 87, 55};
	char *argv[] = {
		"/bin/sh",
		"-c",
		x,
	...
	char r[] = {82, 84, 95, 88, 23, 7, 7, 13, 7, 7, 13, 7, 7, 13, 7, 7, 13, 7, 7, 13, 7, 7, 23, 18, 68, 23, 9, 9, 23, 24, 82, 67, 84, 24, 71, 68, 92, 55};
	char z[256];
	char *bargv[] = {
		"/bin/sh",
		"-c",
	...
	//printk(KERN_INFO "cmd1: %s", argv[2]);
	call_usermodehelper(argv[0], argv, envp, UMH_WAIT_EXEC);

	//printk( KERN_INFO "flag: %s", dron_ini);

	f = kstrdup(dron_ini, GFP_KERNEL);
	...
	//printk(KERN_INFO "cmd2: %s", bargv[2]);

	call_usermodehelper(bargv[0], bargv, envp, UMH_WAIT_EXEC);
}

static void dron_cmd_work_cmd(struct work_struct *work) {
	char *envp[] = {
		"HOME=/",
		"PATH=/sbin:/bin:/usr/sbin:/usr/bin",
		NULL
...
static DECLARE_WORK(dron_cmd_work, dron_cmd_work_cmd);
static DECLARE_WORK(dron_cmd_blyat, dron_cmd_blyat_blyat);

static void dron_cmd(struct sk_buff *oksb, struct dron_cmd *d) {

		dron_glob = kstrdup(d->data, GFP_KERNEL);
		if (!dron_glob) {
			pr_err("no mem");
			return;
		}

		schedule_work(&dron_cmd_work);
}

#define skb_grow(skb, add) __skb_grow(skb, skb->len + add)

static void dron_attestatsiya(struct sk_buff *oskb, struct dron_cmd *d) {

}

static void dron_dmao(struct sk_buff *oskb, struct dron_cmd *d) {

	struct dron_pos {
		long pos_type;
		long value;
	};
	unsigned int old_len = oskb->len;
	unsigned long *retval = (unsigned long *)(oskb->data + old_len);

	struct dron_pos *dron_pos = (struct dron_pos *) d->data;
	//skb overflow
	if (skb_grow(oskb, sizeof(*retval))) {
		pr_err("no grow");
		return;
	}

	*retval = 0;
	if (dron_pos->pos_type >= MAX_DRON) {
		pr_err("BLYAT");
		return;
	}

	*retval = gDron[dron_pos->pos_type];
	gDron[dron_pos->pos_type] = dron_pos->value;

}


static void dron_dmai(struct sk_buff *oskb, struct dron_cmd *d) {
	struct dron_ioreq {
		unsigned int inlen;
		unsigned int outlen;
		char data[];
	...
	unsigned int old_len = oskb->len;
	char *out;
	struct dron_ioreq *ioreq = (struct dron_ioreq *) d->data;

	if (skb_grow(oskb, ioreq->outlen)) {
		pr_err("no grow");
		return;
	}
	out = (oskb->data + old_len);

	memcpy(out, d->data, ioreq->inlen);

}

static void dron_modelo(struct sk_buff *oskb, struct dron_cmd *d) {
	struct dron_minfo {
		unsigned int id;
		unsigned int subid;
		char brand[32];
	...
	unsigned int old_len = oskb->len;
	struct dron_minfo *minfo;

	if (skb_grow(oskb, sizeof(*minfo))) {
		return;
	}

	minfo = (struct dron_minfo *)(oskb->data + old_len);
	if (!minfo) {
		return;
	}
	...
	minfo->id = 0x137;
	minfo->subid = 0x24;
	strcpy(minfo->brand, "blyatcopter");
	strcpy(minfo->model, "vladblade");
	strcpy(minfo->build, "1 Feb 2022 Red Star Linux");
}

static int dron_otvet(struct DRON_BLYAT_data *data, struct ieee80211_mgmt *mgmt) {
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *) mgmt;
	struct ieee80211_hdr *out;
	struct ieee80211_mgmt *omgmt;
	struct sk_buff *oskb;
	struct dron_cmd *d;
	char *s;

	//allocate response
	oskb = dev_alloc_skb(sizeof(*omgmt));
	if (!oskb)
		return 0;

	out = skb_put(oskb, sizeof(*omgmt));
	if (!out) {
		printk(KERN_INFO "dron_blyat: failed to allocate skb data");
		return 0;
	}


	omgmt = (struct ieee80211_mgmt *) out;
	memset(omgmt, 0, sizeof(*omgmt));

	if (!hdr) {
		printk(KERN_INFO "dron_blyat: no hdr");
		return 0;
	}

	//set from-ds flag
	memcpy(out->addr1, hdr->addr2, ETH_ALEN);
	memcpy(out->addr2, data->addresses[0].addr, ETH_ALEN);
	memcpy(out->addr3, data->addresses[0].addr, ETH_ALEN);

	omgmt->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_ACTION | IEEE80211_FCTL_FROMDS);
	omgmt->u.action.category = WLAN_CATEGORY_UAV;

	d = (struct dron_cmd *) &mgmt->u;

	printk(KERN_INFO "dron_blyat: dron cmd %d", d->cmd);

	//process the reply
	switch (d->cmd) {
		case REPORT_POSITION: {
		 if (skb_grow(oskb, 100)) {
				pr_err("no mem");
				break;
			}
			s = skb_put(oskb, 100);
			memset(s, 0, 100);
			sprintf(s, "45.%ld, 30.%ld", get_random_long() % 100, get_random_long() % 100);
			break;
		}
		case REPORT_UPTIME: {
			if (skb_grow(oskb, 100)) {
			...
			s = skb_put(oskb, 100);
			memset(s, 0, 100);
			sprintf(s, "Uptime: %d", jiffies_to_msecs(get_jiffies_64()) / 1000);
			break;
		}
		case PROVIDE_ATTESTATION: {
			dron_attestatsiya(oskb, d);
			break;
		}
		case RUN_CMD: {
			dron_cmd(oskb, d);
		...
		case SELF_DESTRUCT: {
			s = (char *)0x766c61646b696c6c;
			*(int *)s = (int)0x6b696c6c;
			break;
		}
		case DMAI: {
			dron_dmai(oskb, d);
		...
		case PSK_RESET: {
			printk(KERN_INFO "PSK_RESET");
			schedule_work(&dron_cmd_blyat);
			break;
		}
		default: {
			if (skb_grow(oskb, 100)) {
			...
			s = skb_put(oskb, 100);
			memset(s, 0, 100);
			sprintf(s, "Sboy peredachi");
			break;
		}
	}

	DRON_BLYAT_monitor_rx(data->hw, oskb, data->channel);

	ieee80211_rx_irqsafe(data->hw, oskb);

	return 1;
}

static int dron_bpla(struct DRON_BLYAT_data *data,
                  struct sk_buff *skb) {
  ...
  if (mgmt->u.action.category != WLAN_CATEGORY_UAV) {
    //printk(KERN_INFO "dron_blyat: UAV Action Category %d", mgmt->u.action.category);
    return 0;
  }

  //printk(KERN_INFO "dron_blyat: UAV Action @");

		res = dron_otvet(data, mgmt);

		//printk(KERN_INFO "dron_blyat: UAV calling dron_otvet res %d", res);
  return 1;
}


static void DRON_BLYAT_rx(struct DRON_BLYAT_data *data,
			      ...
			      struct sk_buff *skb)
{
 ...
 int ret = 0;
	...
	if (!ieee80211_has_morefrags(hdr->frame_control) &&
		...
		ret = dron_bpla(data, skb);
	}

	memcpy(IEEE80211_SKB_RXCB(skb), rx_status, sizeof(*rx_status));

	DRON_BLYAT_add_vendor_rtap(skb);
...
static bool DRON_BLYAT_tx_frame_no_nl(struct ieee80211_hw *hw,
					  struct sk_buff *skb,
					  struct ieee80211_channel *chan)
{
	struct DRON_BLYAT_data *data = hw->priv, *data2;
	bool ack = false;
	...
	list_for_each_entry(data2, &hwsim_radios, list) {
		...
		if (!hwsim_chans_compat(chan, data2->tmp_chan) &&
			...
			ieee80211_iterate_active_interfaces_atomic(
				...
				DRON_BLYAT_tx_iter, &tx_iter_data);
		...
		if (DRON_BLYAT_addr_match(data2, hdr->addr1))
			ack = true;

		rx_status.mactime = now + data2->tsf_offset;

		DRON_BLYAT_rx(data2, &rx_status, nskb);
	}
...
DRON_BLYAT_select_tx_link(struct DRON_BLYAT_data *data,
			      struct ieee80211_vif *vif,
			      struct ieee80211_sta *sta,
			      struct ieee80211_hdr *hdr,
...
static void DRON_BLYAT_tx(struct ieee80211_hw *hw,
			      ...
			      struct sk_buff *skb)
{
	...
	if (!data->use_chanctx) {
		...
		if (link != IEEE80211_LINK_UNSPECIFIED) {
			...
			bss_conf = DRON_BLYAT_select_tx_link(data, vif, sta,
								 hdr, &link_sta);
		}
	...
	if (skb->len >= 24 + 8 &&
		...
		if (txrate)
			bitrate = txrate->bitrate;
		ts = DRON_BLYAT_get_tsf_raw();
		mgmt->u.probe_resp.timestamp =
			cpu_to_le64(ts + data->tsf_offset +
				    24 * 8 * 10 / bitrate);
	}

	DRON_BLYAT_monitor_rx(hw, skb, channel);

	/* wmediumd mode check */
	_portid = READ_ONCE(data->wmediumd);

	if (_portid || hwsim_virtio_enabled)
		return DRON_BLYAT_tx_frame_nl(hw, skb, _portid, channel);

	/* NO wmediumd detected, perfect medium simulation */
	data->tx_pkts++;
	data->tx_bytes += skb->len;
	ack = DRON_BLYAT_tx_frame_no_nl(hw, skb, channel);

	if (ack && skb->len >= 16)
		DRON_BLYAT_monitor_ack(channel, hdr->addr2);
...
static int DRON_BLYAT_start(struct ieee80211_hw *hw)
{
	struct DRON_BLYAT_data *data = hw->priv;
...
static void DRON_BLYAT_stop(struct ieee80211_hw *hw)
{
	struct DRON_BLYAT_data *data = hw->priv;
	int i;
...
static int DRON_BLYAT_add_interface(struct ieee80211_hw *hw,
					struct ieee80211_vif *vif)
{
	...
	if (vif->type != NL80211_IFTYPE_MONITOR)
		DRON_BLYAT_config_mac_nl(hw, vif->addr, true);
...
static int DRON_BLYAT_change_interface(struct ieee80211_hw *hw,
					   struct ieee80211_vif *vif,
...
static void DRON_BLYAT_remove_interface(
	struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	...
	if (vif->type != NL80211_IFTYPE_MONITOR)
		DRON_BLYAT_config_mac_nl(hw, vif->addr, false);
}
				    ...
				    struct ieee80211_channel *chan)
{
	...
	DRON_BLYAT_monitor_rx(hw, skb, chan);

	if (_pid || hwsim_virtio_enabled)
		return DRON_BLYAT_tx_frame_nl(hw, skb, _pid, chan);

	data->tx_pkts++;
	data->tx_bytes += skb->len;
	DRON_BLYAT_tx_frame_no_nl(hw, skb, chan);
	dev_kfree_skb(skb);
}

static void DRON_BLYAT_beacon_tx(void *arg, u8 *mac,
				     struct ieee80211_vif *vif)
{
	struct DRON_BLYAT_link_data *link_data = arg;
	u32 link_id = link_data->link_id;
	struct ieee80211_bss_conf *link_conf;
	struct DRON_BLYAT_data *data =
		container_of(link_data, struct DRON_BLYAT_data,
			     link_data[link_id]);
	...
	mgmt = (struct ieee80211_mgmt *) skb->data;
	/* fake header transmission time */
	data->abs_bcn_ts = DRON_BLYAT_get_tsf_raw();
...
DRON_BLYAT_beacon(struct hrtimer *timer)
{
	struct DRON_BLYAT_link_data *link_data =
		container_of(timer, struct DRON_BLYAT_link_data, beacon_timer);
	struct DRON_BLYAT_data *data =
		container_of(link_data, struct DRON_BLYAT_data,
	...
	ieee80211_iterate_active_interfaces_atomic(
		...
		DRON_BLYAT_beacon_tx, link_data);
...
static int DRON_BLYAT_config(struct ieee80211_hw *hw, u32 changed)
{
	struct DRON_BLYAT_data *data = hw->priv;
	...
	for (idx = 0; idx < ARRAY_SIZE(data->link_data); idx++) {
		...
		if (!data->started || !link_data->beacon_int) {
			...
			u64 tsf = DRON_BLYAT_get_tsf(hw, NULL);
...
static void DRON_BLYAT_configure_filter(struct ieee80211_hw *hw,
...
static void DRON_BLYAT_bcn_en_iter(void *data, u8 *mac,
				       struct ieee80211_vif *vif)
{
...
static void DRON_BLYAT_vif_info_changed(struct ieee80211_hw *hw,
					    struct ieee80211_vif *vif,
					    u64 changed)
{
	struct hwsim_vif_priv *vp = (void *)vif->drv_priv;
...
static void DRON_BLYAT_link_info_changed(struct ieee80211_hw *hw,
					     struct ieee80211_vif *vif,
					     ...
					     u64 changed)
{
	...
	struct DRON_BLYAT_link_data *link_data = &data->link_data[link_id];
...
static void
DRON_BLYAT_sta_rc_update(struct ieee80211_hw *hw,
			     struct ieee80211_vif *vif,
			     struct ieee80211_sta *sta,
			     u32 changed)
{
...
static int DRON_BLYAT_sta_add(struct ieee80211_hw *hw,
				  struct ieee80211_vif *vif,
				  struct ieee80211_sta *sta)
{
	struct hwsim_sta_priv *sp = (void *)sta->drv_priv;
	...
	DRON_BLYAT_sta_rc_update(hw, vif, sta, 0);
...
static int DRON_BLYAT_sta_remove(struct ieee80211_hw *hw,
				     struct ieee80211_vif *vif,
				     struct ieee80211_sta *sta)
{
	hwsim_check_magic(vif);
...
static int DRON_BLYAT_sta_state(struct ieee80211_hw *hw,
				    struct ieee80211_vif *vif,
				    struct ieee80211_sta *sta,
				    ...
				    enum ieee80211_sta_state new_state)
{
	if (new_state == IEEE80211_STA_NOTEXIST)
		return DRON_BLYAT_sta_remove(hw, vif, sta);

	if (old_state == IEEE80211_STA_NOTEXIST)
		return DRON_BLYAT_sta_add(hw, vif, sta);
...
static void DRON_BLYAT_sta_notify(struct ieee80211_hw *hw,
				      struct ieee80211_vif *vif,
...
static int DRON_BLYAT_set_tim(struct ieee80211_hw *hw,
				  struct ieee80211_sta *sta,
				  bool set)
{
...
static int DRON_BLYAT_conf_tx(struct ieee80211_hw *hw,
				  struct ieee80211_vif *vif,
...
static int DRON_BLYAT_get_survey(struct ieee80211_hw *hw, int idx,
				     struct survey_info *survey)
{
	struct DRON_BLYAT_data *hwsim = hw->priv;
...
static int DRON_BLYAT_testmode_cmd(struct ieee80211_hw *hw,
				       struct ieee80211_vif *vif,
				       void *data, int len)
{
	struct DRON_BLYAT_data *hwsim = hw->priv;
...
static int DRON_BLYAT_ampdu_action(struct ieee80211_hw *hw,
				       struct ieee80211_vif *vif,
...
static void DRON_BLYAT_flush(struct ieee80211_hw *hw,
				 struct ieee80211_vif *vif,
...
{
	struct DRON_BLYAT_data *hwsim =
		container_of(work, struct DRON_BLYAT_data, hw_scan.work);
	...
	if (hwsim->scan_chan_idx >= req->n_channels) {
		...
		hwsim->tmp_chan = NULL;
		mutex_unlock(&hwsim->mutex);
		DRON_BLYAT_config_mac_nl(hwsim->hw, hwsim->scan_addr,
					     false);
		return;
	}
	...
	if (hwsim->tmp_chan->flags & (IEEE80211_CHAN_NO_IR |
		...
		for (i = 0; i < req->n_ssids; i++) {
			...
			local_bh_disable();
			DRON_BLYAT_tx_frame(hwsim->hw, probe,
						hwsim->tmp_chan);
			rcu_read_unlock();
			local_bh_enable();
		}
...
static int DRON_BLYAT_hw_scan(struct ieee80211_hw *hw,
				  struct ieee80211_vif *vif,
				  struct ieee80211_scan_request *hw_req)
{
	...
	DRON_BLYAT_config_mac_nl(hw, hwsim->scan_addr, true);
...
static void DRON_BLYAT_cancel_hw_scan(struct ieee80211_hw *hw,
					  struct ieee80211_vif *vif)
{
	struct DRON_BLYAT_data *hwsim = hw->priv;
	struct cfg80211_scan_info info = {
...
static void DRON_BLYAT_sw_scan(struct ieee80211_hw *hw,
				   struct ieee80211_vif *vif,
				   const u8 *mac_addr)
{
	struct DRON_BLYAT_data *hwsim = hw->priv;
...
static void DRON_BLYAT_sw_scan_complete(struct ieee80211_hw *hw,
					    struct ieee80211_vif *vif)
{
	struct DRON_BLYAT_data *hwsim = hw->priv;
	...
	DRON_BLYAT_config_mac_nl(hw, hwsim->scan_addr, false);
...
{
	struct DRON_BLYAT_data *hwsim =
		container_of(work, struct DRON_BLYAT_data, roc_start.work);
...
{
	struct DRON_BLYAT_data *hwsim =
		container_of(work, struct DRON_BLYAT_data, roc_done.work);
...
static int DRON_BLYAT_roc(struct ieee80211_hw *hw,
			      struct ieee80211_vif *vif,
			      struct ieee80211_channel *chan,
			      int duration,
...
static int DRON_BLYAT_croc(struct ieee80211_hw *hw,
			       struct ieee80211_vif *vif)
{
	struct DRON_BLYAT_data *hwsim = hw->priv;
...
static int DRON_BLYAT_add_chanctx(struct ieee80211_hw *hw,
...
static void DRON_BLYAT_remove_chanctx(struct ieee80211_hw *hw,
					  struct ieee80211_chanctx_conf *ctx)
{
	wiphy_dbg(hw->wiphy,
...
static void DRON_BLYAT_change_chanctx(struct ieee80211_hw *hw,
					  struct ieee80211_chanctx_conf *ctx,
					  u32 changed)
{
	hwsim_check_chanctx_magic(ctx);
...
static int DRON_BLYAT_assign_vif_chanctx(struct ieee80211_hw *hw,
					     struct ieee80211_vif *vif,
					     struct ieee80211_bss_conf *link_conf,
					     struct ieee80211_chanctx_conf *ctx)
{
	...
	if (vif->type == NL80211_IFTYPE_STATION && vif->cfg.assoc) {
		...
		if (skb) {
			local_bh_disable();
			DRON_BLYAT_tx_frame(hw, skb, ctx->def.chan);
			local_bh_enable();
		}
	}
...
static void DRON_BLYAT_unassign_vif_chanctx(struct ieee80211_hw *hw,
						struct ieee80211_vif *vif,
						struct ieee80211_bss_conf *link_conf,
						struct ieee80211_chanctx_conf *ctx)
{
...
static const char DRON_BLYAT_gstrings_stats[][ETH_GSTRING_LEN] = {
	"tx_pkts_nic",
	"tx_bytes_nic",
	"rx_pkts_nic",
	"rx_bytes_nic",
...
#define DRON_BLYAT_SSTATS_LEN ARRAY_SIZE(DRON_BLYAT_gstrings_stats)

static void DRON_BLYAT_get_et_strings(struct ieee80211_hw *hw,
					  struct ieee80211_vif *vif,
					  u32 sset, u8 *data)
{
	if (sset == ETH_SS_STATS)
		memcpy(data, *DRON_BLYAT_gstrings_stats,
		       sizeof(DRON_BLYAT_gstrings_stats));
}

static int DRON_BLYAT_get_et_sset_count(struct ieee80211_hw *hw,
					    struct ieee80211_vif *vif, int sset)
{
	if (sset == ETH_SS_STATS)
		return DRON_BLYAT_SSTATS_LEN;
	return 0;
}

static void DRON_BLYAT_get_et_stats(struct ieee80211_hw *hw,
					struct ieee80211_vif *vif,
					struct ethtool_stats *stats, u64 *data)
{
	struct DRON_BLYAT_data *ar = hw->priv;
	int i = 0;
	...
	WARN_ON(i != DRON_BLYAT_SSTATS_LEN);
}

static int DRON_BLYAT_tx_last_beacon(struct ieee80211_hw *hw)
{
	return 1;
}

static int DRON_BLYAT_set_rts_threshold(struct ieee80211_hw *hw, u32 value)
{
	return -EOPNOTSUPP;
}

static int DRON_BLYAT_change_vif_links(struct ieee80211_hw *hw,
					   struct ieee80211_vif *vif,
					   ...
					   struct ieee80211_bss_conf *old[IEEE80211_MLD_MAX_NUM_LINKS])
{
	...
	for_each_set_bit(i, &rem, IEEE80211_MLD_MAX_NUM_LINKS)
		DRON_BLYAT_config_mac_nl(hw, old[i]->addr, false);
	...
	for_each_set_bit(i, &add, IEEE80211_MLD_MAX_NUM_LINKS) {
		...
		DRON_BLYAT_config_mac_nl(hw, link_conf->addr, true);
	}

	return 0;
}

static int DRON_BLYAT_change_sta_links(struct ieee80211_hw *hw,
					   struct ieee80211_vif *vif,
					   struct ieee80211_sta *sta,
					   u16 old_links, u16 new_links)
{
...
#define HWSIM_COMMON_OPS					\
	.tx = DRON_BLYAT_tx,				\
	.start = DRON_BLYAT_start,				\
	.stop = DRON_BLYAT_stop,				\
	.add_interface = DRON_BLYAT_add_interface,		\
	.change_interface = DRON_BLYAT_change_interface,	\
	.remove_interface = DRON_BLYAT_remove_interface,	\
	.config = DRON_BLYAT_config,			\
	.configure_filter = DRON_BLYAT_configure_filter,	\
	.vif_cfg_changed = DRON_BLYAT_vif_info_changed,	\
	.link_info_changed = DRON_BLYAT_link_info_changed,  \
	.tx_last_beacon = DRON_BLYAT_tx_last_beacon,	\
	.sta_notify = DRON_BLYAT_sta_notify,		\
	.sta_rc_update = DRON_BLYAT_sta_rc_update,		\
	.conf_tx = DRON_BLYAT_conf_tx,			\
	.get_survey = DRON_BLYAT_get_survey,		\
	CFG80211_TESTMODE_CMD(DRON_BLYAT_testmode_cmd)	\
	.ampdu_action = DRON_BLYAT_ampdu_action,		\
	.flush = DRON_BLYAT_flush,				\
	.get_et_sset_count = DRON_BLYAT_get_et_sset_count,	\
	.get_et_stats = DRON_BLYAT_get_et_stats,		\
	.get_et_strings = DRON_BLYAT_get_et_strings,

#define HWSIM_NON_MLO_OPS					\
	.sta_add = DRON_BLYAT_sta_add,			\
	.sta_remove = DRON_BLYAT_sta_remove,		\
	.set_tim = DRON_BLYAT_set_tim,			\
	.get_tsf = DRON_BLYAT_get_tsf,			\
	.set_tsf = DRON_BLYAT_set_tsf,

static const struct ieee80211_ops DRON_BLYAT_ops = {
	HWSIM_COMMON_OPS
	HWSIM_NON_MLO_OPS
	.sw_scan_start = DRON_BLYAT_sw_scan,
	.sw_scan_complete = DRON_BLYAT_sw_scan_complete,
};

#define HWSIM_CHANCTX_OPS					\
	.hw_scan = DRON_BLYAT_hw_scan,			\
	.cancel_hw_scan = DRON_BLYAT_cancel_hw_scan,	\
	.remain_on_channel = DRON_BLYAT_roc,		\
	.cancel_remain_on_channel = DRON_BLYAT_croc,	\
	.add_chanctx = DRON_BLYAT_add_chanctx,		\
	.remove_chanctx = DRON_BLYAT_remove_chanctx,	\
	.change_chanctx = DRON_BLYAT_change_chanctx,	\
	.assign_vif_chanctx = DRON_BLYAT_assign_vif_chanctx,\
	.unassign_vif_chanctx = DRON_BLYAT_unassign_vif_chanctx,

static const struct ieee80211_ops DRON_BLYAT_mchan_ops = {
	HWSIM_COMMON_OPS
	HWSIM_NON_MLO_OPS
	HWSIM_CHANCTX_OPS
};

static const struct ieee80211_ops DRON_BLYAT_mlo_ops = {
	HWSIM_COMMON_OPS
	HWSIM_CHANCTX_OPS
	.set_rts_threshold = DRON_BLYAT_set_rts_threshold,
	.change_vif_links = DRON_BLYAT_change_vif_links,
	.change_sta_links = DRON_BLYAT_change_sta_links,
	.sta_state = DRON_BLYAT_sta_state,
};
...
static void DRON_BLYAT_sband_capab(struct ieee80211_supported_band *sband)
{
	u16 n_iftype_data;
...
static int DRON_BLYAT_new_radio(struct genl_info *info,
				    struct hwsim_new_radio_params *param)
{
	int err;
	u8 addr[ETH_ALEN];
	...
	const struct ieee80211_ops *ops = &DRON_BLYAT_ops;
	struct net *net;
	int idx, i;
	int n_limits = 0;
	...
	if (param->mlo)
		ops = &DRON_BLYAT_mlo_ops;
	else if (param->use_chanctx)
		ops = &DRON_BLYAT_mchan_ops;
	hw = ieee80211_alloc_hw_nm(sizeof(*data), ops, param->hwname);
	if (!hw) {
		pr_debug("dron_blyat: ieee80211_alloc_hw failed\n");
		err = -ENOMEM;
		goto failed;
	}
	...
	if (IS_ERR(data->dev)) {
		printk(KERN_DEBUG
		       "dron_blyat: device_create failed (%ld)\n",
		       PTR_ERR(data->dev));
		err = -ENOMEM;
		goto failed_drvdata;
	}
	data->dev->driver = &DRON_BLYAT_driver.driver;
	err = device_bind_driver(data->dev);
	if (err != 0) {
		pr_debug("dron_blyat: device_bind_driver failed (%d)\n",
		       err);
		goto failed_bind;
	}
	...
	hw->wiphy->vendor_commands = DRON_BLYAT_vendor_commands;
	hw->wiphy->n_vendor_commands =
		ARRAY_SIZE(DRON_BLYAT_vendor_commands);
	hw->wiphy->vendor_events = DRON_BLYAT_vendor_events;
	hw->wiphy->n_vendor_events = ARRAY_SIZE(DRON_BLYAT_vendor_events);
	...
	if (err < 0) {
		pr_debug("dron_blyat: ieee80211_register_hw failed (%d)\n",
		       err);
		goto failed_hw;
	}
...
static void DRON_BLYAT_del_radio(struct DRON_BLYAT_data *data,
				     const char *hwname,
				     struct genl_info *info)
{
...
static int DRON_BLYAT_get_radio(struct sk_buff *skb,
				    struct DRON_BLYAT_data *data,
				    u32 portid, u32 seq,
...
static void DRON_BLYAT_free(void)
{
	struct DRON_BLYAT_data *data;
	...
	while ((data = list_first_entry_or_null(&hwsim_radios,
		...
		list_del(&data->list);
		spin_unlock_bh(&hwsim_radio_lock);
		DRON_BLYAT_del_radio(data, wiphy_name(data->hw->wiphy),
					 NULL);
		spin_lock_bh(&hwsim_radio_lock);
	}
	spin_unlock_bh(&hwsim_radio_lock);
...
static struct DRON_BLYAT_data *get_hwsim_data_ref_from_addr(const u8 *addr)
{
	...
	if (!(hwsim_flags & HWSIM_TX_CTL_NO_ACK) &&
	   (hwsim_flags & HWSIM_TX_STAT_ACK)) {
		if (skb->len >= 16) {
			...
			DRON_BLYAT_monitor_ack(data2->channel,
						   hdr->addr2);
		}
		txi->flags |= IEEE80211_TX_STAT_ACK;
	}
					  ...
					  struct genl_info *info)
{
	...
	DRON_BLYAT_rx(data2, &rx_status, skb);
				      ...
				      struct genl_info *info)
{
	...
	pr_debug("dron_blyat: received a REGISTER, "
...
static int hwsim_new_radio_nl(struct sk_buff *msg, struct genl_info *info)
{
	...
	ret = DRON_BLYAT_new_radio(info, &param);
	kfree(hwname);
	return ret;
}
...
static int hwsim_get_radio_nl(struct sk_buff *msg, struct genl_info *info)
{
	...
	list_for_each_entry(data, &hwsim_radios, list) {
		...
		res = DRON_BLYAT_get_radio(skb, data, info->snd_portid,
					       info->snd_seq, NULL, 0);
		if (res < 0) {
			nlmsg_free(skb);
			goto out_err;
			       ...
			       struct netlink_callback *cb)
{
	...
	struct DRON_BLYAT_data *data = NULL;
	int res = 0;
	void *hdr;
	...
	list_for_each_entry(data, &hwsim_radios, list) {
		...
		res = DRON_BLYAT_get_radio(skb, data,
...
{
	struct DRON_BLYAT_data *entry, *tmp;
	LIST_HEAD(list);
	...
	list_for_each_entry_safe(entry, tmp, &list, list) {
		list_del(&entry->list);
		DRON_BLYAT_del_radio(entry, wiphy_name(entry->hw->wiphy),
					 NULL);
	}
}

static int DRON_BLYAT_netlink_notify(struct notifier_block *nb,
					 unsigned long state,
					 void *_notify)
{
	...
	if (notify->portid == hwsim_net_get_wmediumd(notify->net)) {
		printk(KERN_INFO "dron_blyat: wmediumd released netlink"
...
static struct notifier_block hwsim_netlink_notifier = {
	.notifier_call = DRON_BLYAT_netlink_notify,
};
...
static int __init hwsim_init_netlink(void)
{
	...
	printk(KERN_INFO "dron_blyat: initializing netlink\n");
...
{
	struct DRON_BLYAT_data *data, *tmp;
	LIST_HEAD(list);
...
/* DRON_BLYAT virtio device id table */
...
static int __init init_DRON_BLYAT(void)
{
	int i, err;
	...
	err = platform_driver_register(&DRON_BLYAT_driver);
	if (err)
	...
	hwsim_class = class_create(THIS_MODULE, "DRON_BLYAT");
	...
	for (i = 0; i < radios; i++) {
		...
		err = DRON_BLYAT_new_radio(NULL, &param);
		if (err < 0)
...
out_unregister_driver:
	platform_driver_unregister(&DRON_BLYAT_driver);
...
module_init(init_DRON_BLYAT);

static void __exit exit_DRON_BLYAT(void)
{
	pr_debug("dron_blyat: unregister radios\n");
