...
fn launch_game_1(interpreter: &mut Interpreter,
                 args: &[String],
                 no_sandbox: bool,
...
pub fn play_game_1(challenge_cmd: Vec<String>,
                   no_sandbox: bool,
                   artifacts: Vec<PathBuf>,
                   bytecode: Vec<u8>)
     -> Result<(), Box<dyn Error>> {
...
impl<'a> Interpreter<'a> {
    ...
    pub fn interpret(&mut self, bytecode: &[u8])
         -> Result<(), Box<dyn Error>> {
        info!("Interpreting with Game 1 instruction set!");
        ...
        while self.pc < bytecode.len() {
            ...
            let stream = &bytecode[self.pc..];
            match instruction {
                InstructionEncoding::BitFlipElf => {
                    self.exec_bit_flip_elf(stream)
                }
...
mod test {
    ...
    impl GameTracer for FakeTracer { }
    ...
    fn launch_and_read() -> Result<(), Box<dyn Error>> {
        ...
        launch_game_1(&mut interpreter, &args, true, vec![], &read_stdout)?;
    ...
    fn launch_sh() -> Result<(), Box<dyn Error>> {
        ...
        launch_game_1(&mut interpreter, &args, true, vec![], &interact)?;
    ...
    fn test_budget_exhaustion() -> Result<(), Box<dyn Error>> {
        ...
        let result = interp.interpret(&budget_moves);
